...
exports.updateNewsClick = functions.https.onCall(async (data, _context) => {
  try {
    ...
    if (newsTarget.empty) {
      ...
      const newClickCount = data.isNormalClick ? currentData.click_count + 1 : currentData.click_count;
      const newFavCount = data.isNormalClick ? currentData.fav_count : currentData.fav_count + 1;
      entry.update({
